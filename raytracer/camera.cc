        ...
        // for(int yy=223; yy< 224; yy++){                                        
            ...
            for (int p=0; p<p_samples; p++){
                for (int q=0; q<p_samples; q++){
                    if (p_samples == 1){
                        ray = generateRay(xx, yy);
                    } else {
                        ray = generateRay(xx + (p+get_rand())/p_samples , yy + (q+get_rand())/p_samples);
                    }
                    color += calculateRgb(ray, 0, DBL_MAX, 20, ambient_light, bvh_root, surfaces, lights, materials, 
                        using_bbox, render_bbox, using_bvh, s_samples);
                }
            }

            color /= (p_samples*p_samples);
...
Rgb Camera::calculateRgb(Ray &ray, double min_t, double max_t, 
    ...
    Rgb color;
    Vector rdi = ray.dir.invert();
    for(int i=0; i<lights.size(); i++){
        if (lights[i]->type == 'a'){   

            AreaLight* a_light = dynamic_cast<AreaLight* >(lights[i]);
            Point center = a_light->center;
            Point sampled_light;
            Rgb al_rgb = a_light->rgb;
            double al_len = a_light->len;
            Vector light_dir;
            Rgb al_color;

            Point corner = center - (a_light->u * (al_len/2.) ) - (a_light->v * (al_len/2.) );

            for (int p=0; p < s_samples; p++){
                for (int q=0; q < s_samples; q++){
                    if (s_samples == 1) {
                        sampled_light = center;
                    } else {
                        // double randu = al_len * (get_rand()-.5);
                        // double randv = al_len * (get_rand()-.5);
                        Vector u_scale = a_light->u * al_len * ( (p+get_rand()) /s_samples);
                        Vector v_scale = a_light->v * al_len * ( (q+get_rand()) /s_samples);
                        // Vector v_scale = a_light->v * ( (q+randv) /s_samples);

                        // Vector u_scale = a_light->u * (p )
                
                        // Vector u_scale = a_light->u * al_len * (get_rand()-.5);
                        // Vector v_scale = a_light->v * al_len * (get_rand()-.5);

                        // a_light->v.print();
                        // u_scale.print();
                        // v_scale.print();
                        sampled_light = corner + u_scale + v_scale;
                        // sampled_light.print();

                    }

                    light_dir = sampled_light - closest_ipt;
                    double distance_to_light = light_dir.length();
                    light_dir.normalize();
                    // light_dir.print();

                    Ray shadow_ray (closest_ipt, light_dir, "SHADOW");
                    Rgb shadow_rgb = calculateRgb(shadow_ray, 0.0001, distance_to_light, 1, ambient_light, bvh_root, surfaces, lights, materials, 
                        using_bbox, render_bbox, using_bvh, s_samples);

                    // Vector v = ray.dir.invert();
                    if(!shadow_rgb.isZero()){
                        Rgb li = materials[materialId]->phongShading(rdi, light_dir, al_rgb, closest_intersection.normal);

                        double scale_d = 1/ ( (distance_to_light+1)*(distance_to_light+1) );

                        // double scale_s_p = (a_light->normal).dotProduct(light_dir.invert());
                        double n_dot_nl = (a_light->normal).dotProduct(light_dir.invert());
                        double scale_s_p = (n_dot_nl > 0) ? n_dot_nl : 0;

                        double n_dot_l = (closest_intersection.normal).dotProduct(light_dir);
                        double scale_p_s = (n_dot_l > 0) ? n_dot_l : 0;

                        li *= (scale_d * scale_p_s * scale_s_p);
                        // li *= (scale_p_s);
                        al_color += li;
                    }
                }
            }
            al_color /= (s_samples*s_samples);
            color += al_color;
            ...
            if (!shadow_rgb.isZero()){
                ...
                Rgb li;
                Rgb light_rgb = lights[i]->rgb;

                //pass in lights[i]->getLightDir(closest_ipt) and lights[i]->rgb for all lights but area lights
                li = materials[materialId]->phongShading(v, light_dir, light_rgb, closest_intersection.normal);
        ...
        if(! (materials[materialId]->ideal_specular_c.isZero()) ) {
            ...
            double scale = 2 * (n.dotProduct(rdi));
            Vector r = n*scale - rdi;
            r.normalize();

            Ray reflective_ray (closest_ipt, r, "REFLECTIVE");

            color += ( materials[materialId]->ideal_specular_c * 
                calculateRgb(reflective_ray, 0.0001, DBL_MAX, recurse_limit-1, ambient_light, bvh_root, surfaces, lights, materials, 
                    using_bbox, render_bbox, using_bvh, s_samples) );
        }
    }
