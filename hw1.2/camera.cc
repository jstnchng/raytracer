...
Rgb Camera::shade (Ray ray, P_Light light, Intersection intersection){
	Vector normal = intersection.normal;
	Material mat = intersection.material;
	Point ipt = intersection.get_closer_ipt();

	Point light_pos = light.pos;
	Vector light_dir = light_pos - ipt;
	light_dir.normalize();
	...
	Rgb kd_I = mat.diffuse_c * light.rgb;
	l_diffuse = kd_I * n_ld;
	...
	if (h.length() > 0.){
		...
		n_h = max(0.0, n_h);
		n_h = pow(n_h, mat.phong_exp);

		Rgb ks_I = mat.specular_c * light.rgb;
		l_specular = ks_I * n_h;
	}
...
void Camera::renderScene (std::vector< Surface * > &surfaces, P_Light light){
	...
	for(int xx=0; xx< pixels_width; xx++){
		for(int yy=0; yy< pixels_height; yy++){		
        	...
        	Rgb color = shade(ray, light, closest_intersection);
