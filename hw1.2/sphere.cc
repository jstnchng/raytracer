...
Intersection Sphere::intersect(Ray &ray) {
	...
	assert(intersection_t1 >= 0.0);
	assert(intersection_t2 >= 0.0);
	...
	return Intersection(does_intersect, intersection_t1, intersection_t2, intersection_pt1, intersection_pt2, normal, material);
}

void Sphere::init(Point o, float r, Material m){
	origin = o;
	radius = r;
	material = m;
}
