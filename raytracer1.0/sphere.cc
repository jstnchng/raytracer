...
Intersection Sphere::intersect(Ray &ray, bool using_bbox, bool render_bbox) {
	...
	return Intersection(does_intersect, intersection_t1, intersection_t2, intersection_pt1, intersection_pt2, normal, materialIndex);
}
...
void Sphere::set_bbox(){
	double scale = 0.0001 + radius;

	Point min(origin[0]-scale, origin[1]-scale, origin[2]-scale);
	Point max(origin[0]+scale, origin[1]+scale, origin[2]+scale);
	bbox.init(min, max);
	has_bbox = true;
}
